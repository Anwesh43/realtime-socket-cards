...
class RealTimeCard{constructor(text,color,textColor){this.text=text
...
this.textComponents=[]
this.color=color}
createTextComponents(canvas,context){var msg=""
var y=this.w/10,x=this.w/10
for(let token of this.text.split(" ")){const wText=context.measureText(msg+token).width
if(wText>4*this.w/5){const wMsg=context.measureText(msg).width
...
updateTopPosition(gap){this.img.style.top=parseFloat(this.img.style.top)+gap}
create(){const canvas=document.createElement('canvas')
...
this.textComponents.forEach((textComponent)=>{textComponent.draw(context)})
this.img.src=canvas.toDataURL()}}
class TextComponent{constructor(text,textColor,x,y){this.x=x
this.y=y
this.text=text
this.textColor=textColor}
draw(context){console.log(this.text)
context.fillStyle=this.textColor
context.fillText(this.text,this.x,this.y)}}
class RealTimeCardFactory{constructor(color,textColor){this.cards=[]
this.color=color
this.textColor=textColor}
addCard(text){const realtimeCard=new RealTimeCard(text,this.color,this.textColor)
realtimeCard.create()
const gap=realtimeCard.getHeight()
this.cards.forEach((card,index)=>{card.updateTopPosition(gap+window.innerHeight/10)})
this.cards.push(realtimeCard)}}
class RealTimeCardStore{static init(url,action,cardColor,cardTextColor){const realTimeCardFactory=new RealTimeCardFactory(cardColor||"black",cardTextColor||"white")
if(io){const socket=io(url)
socket.on(action,(data)=>{realTimeCardFactory.addCard(data)})}}}
